...
######--------Testing
svm = cv2.ml.SVM_load('svm_data2.dat')
test_img_path = test_img_home + 'img_11.jpg'
test_img = cv2.imread(test_img_path,0)
test_data = np.float32(test_img)
...
match = [x for x,y in zip(result,correct_response) if x == y]
print len(match)*100.0/len(result)
"""
